...
$sql_auth="SELECT * FROM $auth_table INNER JOIN history_images on $auth_table.article_id= history_images.article_id WHERE history_images.main = 1 and $auth_table.user_id ='$user_id' ORDER BY $auth_table.created_date DESC Limit 3";

$sql_result_auth=mysqli_query($con, $sql_auth);

?>
                <div class="d-flex justify-content-between w-100 mt-5 pt-2">
                <?php if ($lang_dir == "en") : ?>
                    <div class='items'>
                    ...
                    <?php if(mysqli_num_rows($sql_result_auth)>0){?>
                    <!--<div class="nav-link" data-toggle="pill" href="#myAuth_all"> all articles</div>-->
                    <?php } ?>
                <?php endif; ?>
                <?php if ($lang_dir == "ru") : ?>
