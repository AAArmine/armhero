 <?php
                    ...
                    $sql_auth="SELECT * FROM $auth_table INNER JOIN autobiography_images on $auth_table.article_id= autobiography_images.article_id WHERE autobiography_images.main = 1 and $auth_table.user_id ='$user_id' ORDER BY $auth_table.created_date DESC Limit 3";
                ...
                <?php if ($lang_dir == "en") : ?>
                    <div class='items'>
                        <h5 class="sel_article"> My Biographies </h5>
                    </div>
                    <div><a href="../<?php echo $lang_dir ?>/add-article.php" class="create_article"
                    id="create_link">Create a Biography <i class="fas fa-plus"></i></a></div>
                <?php endif; ?>
                <?php if ($lang_dir == "ru") : ?>
                    <div><h5 class="sel_article"> Мои биографии</h5></div>
                    <div><a href="../<?php echo $lang_dir ?>/add-article.php" class="create_article"
                    id="create_link">Создать биографию <i class="fas fa-plus"></i></a></div>
                <?php endif; ?>
                <?php if ($lang_dir == "am") : ?>
                    <div><h5 class="sel_article">Իմ կենսագրականներ</h5></div>
                    <div><a href="../<?php echo $lang_dir ?>/add-article.php" class="create_article"
                    id="create_link">Ստեղծել կենսագրություն <i class="fas fa-plus"></i></a></div>
                <?php endif; ?>
                </div>
                <div class="d-flex justify-content-around w-100 flex-wrap">
                    ...
                    <div class="mt-3 col-8 col-sm-8 col-md-8 col-lg-8 col-xl-8" id="no_result">
                        ...
                        <?php if ($lang_dir == "en") : ?>
                            <h3 class="sel_article"> You do not have active Biographies </h3>
                        <?php endif; ?>
                        <?php if ($lang_dir == "ru") : ?>
                            <h3 class="sel_article">У вас нет активных Биографий</h3>
                        <?php endif; ?>
                        <?php if ($lang_dir == "am") : ?>
                            <h3 class="sel_article">Դուք չունեք ակտիվ կենսագրություններ</h3>
                        <?php endif; ?>
