    ...
    <?php
           ...
           if(isset($_GET['article'])){
                ...
                if($article_category=='autobiography'){
                    $sql_all = "SELECT a.name, c.id, c.description, c.date, u.fullname FROM ".$article_category."_articles_default as a 
                INNER JOIN ".$article_category."_edited_articles_by_users as c ON c.article_id=a.article_id 
                INNER JOIN users as u ON u.id=c.user_id WHERE c.article_status=0";
                }else{
                    $sql_all = "SELECT a.title, c.id, c.description, c.date, u.fullname FROM ".$article_category."_articles_default as a 
                INNER JOIN ".$article_category."_edited_articles_by_users as c ON c.article_id=a.article_id 
                INNER JOIN users u ON u.id=c.user_id WHERE c.article_status=0";
                }
                
                $sql_all_result=mysqli_query($con, $sql_all);
                                                  // var_dump($sql_all_result);
        ...
        ?>
            ...
            <!-- End Navbar -->
            <div class="content" data-type='addition'>
                                                    ...
                                                    <div class="form-group">
                                                      <label>Ընտրել հոդվածի տեսակը</label>
                                                        <select onchange="location = this.value;" class="form-control select" id="select_categories" name='sel-categories'>
                                                            <option value=""></option>
                                                            <option value='new-addition.php?article=history' <?php if(isset($article_category)&&($article_category=="history")){
                                                                echo 'selected';
                                                            }?>>Հոդված</option>
                                                            <option value='new-addition.php?article=autobiography' <?php if(isset($article_category)&& ($article_category=="autobiography")){
                                                                echo 'selected';
                                        ...
                                        <table id="datatables" data-name="default" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                                            ...
                                            ?>
                                                <tr data-id="">
                                                    ...
                                                    <td class='c-date'><?php echo $sql_all['date']; ?></td>
                                                    <td class='text-left d-flex flex-column' data-article-id=''>
                                                        <button class='delete-addition btn btn-danger open-modal-delete-comment' id="delete_<?php echo $sql_all['id']; ?>" >Ջնջել</button>
                                                        <button class='confirm-addition btn btn-succes' id="confirm_<?php echo $sql_all['id']; ?>" >Հաստատել</button>
                                                    </td>
                                                </tr>
                                                <?php
                                                $k++;
            ...
            <script src="../js/newAddition.js"> </script>     
</body>
</html>